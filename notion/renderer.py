...
class BaseHTMLRenderer(BaseRenderer):
    ...
    def __init__(
        ...
        render_table_pages_after_table=False,
        with_styles=False,
    ):
        """
    ...
    def render(self, **kwargs):
        ...
        return (HTMLRendererStyles if self.with_styles else "") + "".join(
            el.render(**kwargs) if isinstance(el, dom_tag) else el for el in els
        )
    ...
    def render_blocks_into(self, blocks, containerEl=None):
        if (
            containerEl is None
        ):  # Default behavior is to add a container for the children
    ...
    def render_column_list(self, block):
        return self.render_blocks_into(
    ...
    def render_page(self, block):
        ...
        ):  # If it's a child of a collection (CollectionRowBlock)
    ...
    def render_bulleted_list(self, block):
        ...
        containerEl = (
            previousSibling if previousSiblingIsUl else ul()
        )  # Make a new ul if there's no previous ul
        ...
        return (
            [] if containerEl.parent else [containerEl]
    ...
    def render_numbered_list(self, block):
        ...
        containerEl = (
            previousSibling if previousSiblingIsOl else ol()
        )  # Make a new ol if there's no previous ol
    ...
    def render_tweet(self, block):
        ...
        return requests.get(
            "https://publish.twitter.com/oembed?url=" + block.source
        ).json()["html"]
    ...
    def render_callout(self, block):
        return [
            div(
                div(block.icon, _class="icon")
                + div(renderMD(block.title), _class="text"),
                _class="callout",
            )
        ]
